...
class TLSClientApp:
    def __init__(self, master):
        self.master = master
        Client_ver = "01.00.02" # Updated version number
        ...
        if self.ssl_conn and self.client_connected: # Check self.client_connected ensures handshake completed
            try:
                ...
                if e.args and isinstance(e.args[0], (list, tuple)) and all(isinstance(item, tuple) and len(item) == 4 for item in e.args[0]):
                    ...
                    self.log_message(f"Error during SSL connection shutdown (might be expected if connection reset): {e}\nDetails:\n" + "\n".join(error_details), "warning")
                else:
                    self.log_message(f"Error during SSL connection shutdown: {e} (No detailed OpenSSL error stack available)", "warning")
            except Exception as e:
                self.log_message(f"Unexpected error during SSL shutdown: {e}", "warning")
            finally:
                try:
                    self.ssl_conn.close()
                except OSError as e:
                    self.log_message(f"Error closing SSL connection: {e}", "error")
                self.ssl_conn = None
    ...
    def _client_thread(self, ip_address, port, cert_file, key_file, ca_file, tls_version):
        ...
        try:
            ...
            # Loop until handshake is successful, fatal error occurs, or disconnect is signaled
            ...
            if e.args and isinstance(e.args[0], (list, tuple)) and all(isinstance(item, tuple) and len(item) == 4 for item in e.args[0]):
                for err_code, lib, func, reason in e.args[0]:
                    error_details.append(f"  - Library: {lib}, Function: {func}, Reason: {reason}")
                self.log_message(f"SSL Error during connection setup: {e}\nDetails:\n" + "\n".join(error_details), "error")
            else:
                self.log_message(f"SSL Error during connection setup: {e} (No detailed OpenSSL error stack available)", "error")
            self.disconnect_client()
        except Exception as e:
